...
- A stamp exists only if the server fetched the transaction itself and it satisfies all six
  conditions in the [trust model](/docs/concepts/trust-model).
...
- [Threat model](/docs/security/threat-model): every attack, what stops it, and the proof, including the
  weaknesses we chose not to fix.
- [Audits](/docs/security/audits): who checked this and how, stated without dressing it up.
- [The prove-it command](/docs/developers/the-prove-it-command): six of these attacks run against
