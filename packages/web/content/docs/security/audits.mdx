...
fix, in plain words. Read it at [threat model](/docs/security/threat-model).
...
[the prove-it command](/docs/developers/the-prove-it-command), so you can compare what we claim
